...
def start_schema_worker():
    ...
    try:
        ...
        worker_process = subprocess.Popen(
            [sys.executable, "-m", "core.worker.listen_and_refresh"], cwd=os.getcwd()
        )

        logging.info(
            f"✅ Schema worker started as separate process (PID: {worker_process.pid})"
        )
...
def check_environment():
    ...
    if base_db_url == "postgresql+asyncpg://user:password@localhost/":
        ...
        logging.info(
            "   For production, set BASE_DATABASE_URL=postgresql+asyncpg://user:pass@localhost/"
        )
        logging.info(
            "   Schema worker will start but may not connect to real databases."
        )
    else:
...
if __name__ == "__main__":
    ...
    logging.info(
        "💡 To start schema worker manually: python -m core.worker.listen_and_refresh"
    )
