...
class DBEmbedder:
    ...
    async def _ensure_pgvector_table(self):
        ...
        try:
            ...
            await conn.execute("""
                               CREATE
                               OR REPLACE FUNCTION ddl_notify_schema_change()
                ...
                DECLARE
                    ...
                    sch_name
                               ...
                               BEGIN
                                ...
                                obj_name
                               := COALESCE(cmd.object_identity, cmd.objid::text, 'unknown');
                                sch_name
                               := COALESCE(cmd.schema_name, 'public');

                                -- Skip if this is the schema_embeddings table itself
                                IF
                               obj_name LIKE '%schema_embeddings%' THEN
                                    CONTINUE;
                               END IF;
                               ...
                               := json_build_object(
                                ...
                                PERFORM
                               pg_notify('schema_changed', payload::text);
    ...
    async def _similarity_search_async(self, query: str, k: int = 6) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"🔍 Debug - Available tables for {self.db_name}: {[(r['schema'], r['table']) for r in debug_tables]}")

            # Search using cosine similarity (1 - cosine_distance)
            results = await conn.fetch(
                """
                SELECT
                    schema, "table", (1 - (embedding <=> $1::vector)) AS similarity_score
